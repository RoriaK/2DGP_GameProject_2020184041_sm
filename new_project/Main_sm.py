...
#화면 크기 설정
screen_width = 640 #배경 가로
screen_height = 480 #배경 세로
...
#공 크기에 따른 최초 스피드 (공이 클 수록 스피드가 빨라야 함)
...
def crash_ball():
    global running, weapon_to_remove, ball_to_remove
    # 4. 충돌 처리

    # 캐릭터 rect 정보 업데이트(충돌처리)
...
def handle_events():
    global weapon_x_pos, weapon_y_pos, character_to_x, ball_to_remove
    # 2. 이벤트 처리(키보드, 마우스 등)
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYUP:  # 떼었을 때
...
def update():
    global running, character_x_pos, weapons, weapon_to_remove, ball_to_remove, weapon_x_pos, weapon_y_pos
    # 3. 게임 캐릭터 위치 정의
...
def draw():
    global weapon_x_pos, weapon_y_pos
    clock.tick(30)  # 게임 화면의 초당 프레임 수를 설정
